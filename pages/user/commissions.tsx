...
const CommissionPage: NextPage = () => {
    ...
    const nextStep = () => {
        ...
        if (selectedRow.length===0) {
            setToast({ id: Math.random(), message: notification.notSelectedItem, alertColor: 'info' });
            return;
        }
                    ...
                    <Card dir={settings.direction}>
                        ...
                        <CardContent>
                            <CenterBox sx={{ display: loadingText === '' ? 'flex' : 'none' }}>
                                <Tabs value={activeTab} onChange={handleTabChange} aria-label='advance-commissions-tabs'>
