...
const Handler = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    let { address, price } = <{ address: string; price: number; }>req.body;

    const maxAddressLength = 800;
    address = address.substring(0, maxAddressLength);

    if (arrayHasNullOrEmptyItem([agencyID, price, location, address]))
    ...
    try {
        ...
        const priceItem = await prisma.prices.create({
            data: {
                address: address,
                latitude: location[0],
        ...
        if (!priceItem)
            return res.status(503).json({ error: 'ERR_ITEM_NOT_CREATED' });
