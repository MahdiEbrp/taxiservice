...
const Handler = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    try {
        ...
        const personelInfo = await prisma.user.findMany({
            where: {
                id: {
                    in: personel.map((p) => p.userId)
                }
            }
        });
        const combined = personel.map((p) => {
            const a = agency.find((a) => a.id === p.agencyId);
            const u = personelInfo.find((u) => u.id === p.userId);
