...
const Handler = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    try {
        ...
        const myAgencies = await prisma.agency.findMany({
            where: {
                userId: user.id
            },
        });
        const allAgencyNames = await prisma.agency.findMany({
            select: {
                agencyName: true,
            }
        });
        ...
        return res.status(200).json({ myAgencies: myAgencies, allAgencyNames: allAgencyNames });
    }
    catch (error) {
